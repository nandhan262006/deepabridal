...
const services = [
  { icon: Scissors, title: "Bridal Hair", subtitle: "Styling & Adornment",
    desc: "From intricate updos adorned with flowers and jewels to flowing romantic waves — every strand crafted to complement your bridal look.",
    items: ["Bridal Updo","Mehendi Hair","Reception Look","Hair Extensions"] },
  { icon: Star, title: "Skincare", subtitle: "Glow & Radiance",
    desc: "Pre-bridal skin rituals that prepare your canvas for the most photographed day of your life. We ensure your skin is luminous and flawless.",
    items: ["Bridal Facials","De-Tan Treatments","Cleanup & Polishing","Glow Packages"] },
  { icon: Sparkles, title: "Bridal Makeup", subtitle: "Artistry & Elegance",
    desc: "Signature makeup tailored to your skin tone, attire, and personality. Long-lasting, HD-ready finishes for every function.",
    items: ["Engagement Makeup","Wedding Day Look","Reception Glam","Saree Draping"] },
  { icon: Gem, title: "Nail Art", subtitle: "Details & Design",
    desc: "Intricate nail art and gel manicures designed to complement your bridal jewelry — from classic french to ornate floral patterns.",
...
function ServiceCard({ service, index }) {
  ...
  return (
    <motion.div ref={ref}
      ...
      className="relative group border border-yellow-800/30 hover:border-yellow-600/50 transition-all duration-500 p-8 overflow-hidden"
      ...
      <div className="relative z-10">
        <div className="mb-5 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="font-sans text-[9px] tracking-[0.3em] uppercase text-yellow-600/55">{service.subtitle}</p>
          </div>
        </div>
        <div className="divider-gold mb-4 opacity-25" />
        <p className="font-body text-sm text-yellow-100/55 leading-relaxed mb-5">{service.desc}</p>
        <ul className="space-y-1.5">
          {service.items.map(item=>(
            <li key={item} className="flex items-center gap-3">
              <span className="w-1 h-1 rounded-full bg-yellow-500 flex-shrink-0" />
              <span className="font-sans text-xs tracking-[0.12em] text-yellow-300/65">{item}</span>
            </li>
          ))}
        </ul>
      </div>
...
export default function Services() {
  ...
  return (
    <section id="services" className="deepa-bg-dark relative py-28 px-6 overflow-hidden">
      <div className="relative z-10 max-w-6xl mx-auto">
        <motion.div ref={ref} initial={{opacity:0,y:30}} animate={inView?{opacity:1,y:0}:{}} transition={{duration:0.8}} className="text-center mb-14">
          <p className="font-sans text-[10px] tracking-[0.6em] uppercase text-yellow-600/55 mb-4">What We Offer</p>
          <h2 className="font-display text-4xl md:text-5xl gold-text mb-5">Our Services</h2>
          <div className="flex items-center justify-center gap-4">
            <div className="h-px w-20 bg-gradient-to-r from-transparent to-yellow-700" />
            <span className="font-script text-xl text-yellow-400/40">crafted with love</span>
